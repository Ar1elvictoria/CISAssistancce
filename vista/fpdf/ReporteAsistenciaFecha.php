...
if (!empty($_GET["fechainicio"]) and !empty($_GET["fechafinal"]) and !empty($_GET["namepracticante"])) {

   $fechainicio=$_GET["fechainicio"];
   $fechafinal=$_GET["fechafinal"];
   $practicante=$_GET["namepracticante"];
   ...
   class PDF extends FPDF
   {
      ...
      function Header()
      {
         ...
         $this->Cell(100, 10, utf8_decode("REPORTE DE ASISTENCIAS POR FECHA"), 0, 1, 'C', 0);
         $this->Ln(7);
         ...
         $this->Cell(30, 10, utf8_decode('TOTAL_HORAS'), 1, 1, 'C', 1);
      }
   ...
   if ($practicante == "todos") {
      ...
      where entrada BETWEEN '$fechainicio' and '$fechafinal' order by id_practicante asc ");
   } else {
      $sql=$conexion->query("SELECT 
      asistencia.id_asistencia, 
      asistencia.id_practicante, 
      date_format(asistencia.entrada, 'n-%d-%Y %H:%i:%s') as 'entrada', 
      date_format(asistencia.salida, 'Sm-sd-%Y %H:%i:%s') as 'salida', 
      TIMEDIFF(asistencia.salida,asistencia.entrada) as 'totalHR', 
      practicante.nombre, 
      practicante.apellidop, 
      practicante.apellidom, 
      practicante.uak, area.nombre AS 'area' 
      FROM asistencia 
      INNER JOIN practicante ON asistencia.id_practicante = practicante.id_practicante 
      INNER JOIN area ON practicante.area = area.id_area 
      where asistencia.id_practicante=$practicante and entrada BETWEEN '$fechainicio' and '$fechafinal' 
   ...
   while ($datos_reporte = $sql->fetch_object()) {
      ...
      $pdf->Cell(80, 10, utf8_decode($datos_reporte->nombre . " " . $datos_reporte->apellidop . " " . $datos_reporte->apellidom), 1, 0, 'C', 0);
      $pdf->Cell(50, 10, utf8_decode($datos_reporte->area), 1, 0, 'C', 0);
